...
class BonTransportView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if dataInvoice: 
           ...
           if adresse == 'Mohammadia Mall' :        
                responsable_entrepot = CustomUser.objects.filter(username='ziad').first()
                if responsable_entrepot:
                    notify.send(
                        sender=myuser,
                        recipient=responsable_entrepot,
                        verb=f'Bon de Livraison Numero {dataInvoice["bonlivraison"]} est en route vers Mohammadia Mall',
...
class BonTransportListView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        moyens_transport = models.MoyenTransport.objects.filter(store=selected_store)
        context["entrepots"] = moyens_transport
        users_bills = CustomUser.objects.filter(EmployeeAt=selected_store)
        context["users"]=users_bills
        return context 
